...
**Artifact:** `skills/parsing-compliance-certificates/SKILL.md` (plus two reference files)

### Sub-checks

| Sub-check | Verdict | Evidence |
| --- | --- | --- |
| YAML frontmatter `name` is lowercase + hyphens, ≤64 chars | PASS | `parsing-compliance-certificates` (32 chars). Verified by regex against the file. |
| `name` does not contain reserved words ("anthropic", "claude") | PASS | Verified. |
| `name` uses gerund form | PASS | "parsing" is the gerund. |
