...
## 2026-05-15 — LINT — HITL state-count correction (rescued from superseded worktree)

- Files touched: `wiki/llm-integration/hitl-state-machine.md` (prose "seven states" → "eight states" in the lede and the section heading; `last_updated` 2026-05-13 → 2026-05-15); `wiki/index.md` (hitl-state-machine row: "Seven review states" → "Eight review states", provenance marker `seed` → `lint`).
- Trigger: worktree cleanup. The `worktree-wiki-lint-pass` branch (`aefa27d`, based on stale `610fa7a`) ran in parallel with the lint pass that landed on `main` as `9d3b403`. Most of its changes were superseded by `9d3b403`, but it carried one correct, unmerged fix not present anywhere on `main`/`origin`: the page's prose said "seven states" while the diagram and state table both enumerate eight (AI_DRAFT, VALIDATION_PASSED, PENDING_REVIEW, IN_REVIEW, APPROVED, REJECTED, REVISION_REQUESTED, ESCALATED). Verified against the state table before applying; the rest of the stale branch was discarded.
- No source re-ingest; prose-only correctness fix. Page count unchanged.

## 2026-05-15 — UPDATE — Production skills conformed into `skills/`; naming-divergence flag resolved (with LINT reconciliation)

- Files touched:
  - **Updated pages (6):**
    - `wiki/methodology/skill-naming-convention.md` — "Production divergence — maintainer flag" section reframed to "Production provenance — repo gap resolved" (gap closed at the repo level: `skills/` now holds five conforming `dl-*` bundles; `ol-`/freeform names remain only as Claude Desktop deployment provenance pending fleet redeploy); Related Concepts `production-skill-inventory` line reworded off the open-gap framing; `last_updated` 2026-05-13 → 2026-05-15. **Domain-registry table not touched** (orchestrator-owned; already updated with `sector`/`prompt`).
    - `wiki/production-skills/production-skill-inventory.md` — "four deployed skills" table + sourcing-to-posting prose re-keyed to repo-conformant `dl-*` names with `(production: ol-…)` provenance; "Maintainer flag — naming-convention divergence" section reframed to "Naming-convention divergence — repo gap resolved" mirroring page 1; Related Concepts divergence line reworded. `last_updated` already 2026-05-15.
    - `wiki/production-skills/sector-research-screener.md` — lede repointed `ol-industry-screener` → `dl-sector-screen` (production name kept as provenance). `last_updated` already 2026-05-15.
    - `wiki/production-skills/posting-memo-automation.md` — lede + both section headings repointed `overland-posting-memo` → `dl-memo-posting`, `populating-posting-memo-backup` → `dl-memo-posting-backup` (production names kept as provenance). `last_updated` already 2026-05-15.
    - `wiki/production-skills/prompt-generator-skill.md` — lede repointed `ol-prompt-generator` → `dl-prompt-generate` (production name kept as provenance). `last_updated` already 2026-05-15.
    - `wiki/deal-lifecycle/origination-and-screening.md` — Related-Concepts parentheticals updated to `dl-*` repo names with production provenance; `last_updated` 2026-05-13 → 2026-05-15 (edited).
  - **Index:** `wiki/index.md` — three production-skills row summaries re-keyed to `dl-*` (production provenance noted); inventory-row summary off the divergence-flag framing; `skill-naming-convention` row summary corrected `16-domain` → `18-domain` registry (orchestrator added `sector`, `prompt`) and provenance marker `ingest` → `lint`. No page added or removed; Categories table and page counts unchanged.
- Assessed, no change needed:
  - `wiki/methodology/overland-credit-framework.md` — body uses `[[wikilinks]]` only; the only `ol-`/old-name hits are immutable `raw/` `.zip` archive filenames in `sources:` frontmatter and the `## Sources` block (`ol-prompt-generator.zip`, `ol-industry-screener.zip`, `overland-posting-memo.zip`) — not renamed per the archive-immutability rule. Page left unchanged.
- Name reconciliation (production deployment name → repo-conformant name, kept paired as provenance everywhere):
  - `ol-industry-screener` → `dl-sector-screen`
  - `overland-posting-memo` → `dl-memo-posting`
  - `populating-posting-memo-backup` → `dl-memo-posting-backup`
  - `ol-prompt-generator` → `dl-prompt-generate`
- Divergence-flag resolution: the previously-recorded documented-convention-vs-live-fleet **governance gap is resolved at the repo level** — verified `skills/` holds `dl-compcert-review`, `dl-sector-screen`, `dl-memo-posting`, `dl-memo-posting-backup`, `dl-prompt-generate` (five conforming `dl-*` bundles). Reframed honestly as resolved-at-repo, NOT as a claim the Claude Desktop fleet has been redeployed; deployment names retained throughout as production provenance pending redeploy (a maintainer operation tracked outside the wiki). No `⚠️ CONTRADICTION` callout warranted — no page-vs-page factual contradiction existed (the literal "every skill in `skills/`…" claim was already true and is now true with the fuller fleet).
- Authorization: maintainer-approved "push 1" plan (`plan/push-1-plan-raw-skills-performance.md`, APPROVED) — satisfies the charter's substantive-edit approval gate.
- Out of scope / untouched: the `skill-naming-convention.md` **domain-registry table** is orchestrator-owned and was already correctly updated with the `sector` and `prompt` domains — the wiki-editor did not edit it. No `raw/`, `docs/`, `skills/`, `prompts/`, `project-instructions/`, `schemas/`, or `CLAUDE.md` writes. No git operations. No agents spawned.
- New categories: none. New tags: none. Page count unchanged.
- `progress.json`: `wiki.last_lint_date` set to 2026-05-15 (atomic, sole field touched by the wiki-editor this operation).

## 2026-05-15 — LINT — Full lint pass post-push-1; clean across all six checks

- Files touched: `wiki/log.md` (this entry only). No wiki page edited; no cosmetic repair required.
- New categories: none. New tags: none. Page count unchanged (51).
- Scope: full sweep of all 51 topical pages plus `index.md` reconciliation, against WIKI-SCHEMA §5. Targeted attention per maintainer request to the push-1 production-skill-name reconciliation and the HITL review-state-count claim.
- Findings — all checks clean, no action required:
  - **Broken wikilinks:** clean. Extracted every `[[target]]` across all pages; all resolve to one of the 51 existing page slugs. The only non-resolving `[[...]]` tokens are the prose/format examples inside `WIKI-SCHEMA.md` (`slug`, `link`, `wikilink`, `wikilinks`, `another-page-slug`, `other-page-slug`, `Stoplight Rating`, `stoplight-rating`) — schema documentation, not content links. Zero broken links in topical pages.
  - **Orphans:** none. Every page has ≥2 inbound topical `[[wikilinks]]` (excluding the catalog files and self-references). Lowest inbound: `portco-coverage-input-schema` and `prompt-generator-skill` at 2 each — appropriate density for second-tier schema / single-skill detail pages.
  - **Missing sources:** none. All 51 pages declare a non-empty `sources:` frontmatter list and carry a non-empty `## Sources` section.
  - **Schema violations:** none. All 51 pages carry the five required frontmatter fields (`title`, `category`, `tags`, `sources`, `last_updated`), both `## Related Concepts` and `## Sources` sections, and a `category:` value that matches the containing folder on every page.
  - **Stale pages:** none. Staleness threshold for today (2026-05-15) is `last_updated` earlier than 2025-11-16; every page is dated 2026-05-13 or 2026-05-15, well inside the 180-day window.
  - **Index / file reconciliation:** clean. 51 on-disk topical pages, 51 `index.md` catalog rows, `progress.json.wiki.page_count` = 51 — all three agree. Zero dangling index rows (no row points to a missing page), zero missing rows (no file lacks a row), zero duplicate rows. The Categories table sums exactly to 51 and every per-category count matches the on-disk file count (deal-lifecycle 4, pain-points 3, opportunities 2, economics 4, strategic-options 3, arrakis-architecture 3, data-substrate 3, llm-integration 5, governance 2, library-design 2, methodology 2, deal-templates 14, production-skills 4). No reconciliation drift — the +1 offset closed by the lint-2 entry remains closed; the prior production-skills INGEST introduced no new offset.
  - **Contradictions — push-1 production-skill name reconciliation:** clean. `skill-naming-convention.md` ("Production provenance — repo gap resolved") and `production-skill-inventory.md` ("Naming-convention divergence — repo gap resolved") both state the divergence is resolved at the repo level with deployment names retained as provenance; the `index.md` row summaries (`production-skill-inventory`, `skill-naming-convention`, the three skill rows) match that resolved framing. No page asserts the unresolved divergence flag while another asserts it resolved. No `⚠️ CONTRADICTION` callout warranted.
  - **Contradictions — HITL review-state count:** clean. `index.md` summarizes `hitl-state-machine` as "Eight review states"; the page lede says "the eight states," the section heading is "## The eight states," and the state table enumerates exactly eight (AI_DRAFT, VALIDATION_PASSED, PENDING_REVIEW, IN_REVIEW, APPROVED, REJECTED, REVISION_REQUESTED, ESCALATED). No residual "seven states" / "7 states" claim anywhere in `wiki/`. No page citing a HITL state count disagrees. The 2026-05-15 "seven→eight" correction (rescued from the superseded worktree) is fully propagated and internally consistent.
